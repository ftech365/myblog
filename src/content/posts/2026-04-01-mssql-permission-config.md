...
- [ServerRole — サーバーレベルのロール](#serverrole--サーバーレベルのロール)
  - [従来の固定サーバーロール（2019以前）](#固定サーバーロール一覧)
  - [SQL Server 2022で追加された固定サーバーロール（##MS_系）](#sql-server-2022で追加された固定サーバーロールms_系)
...
SQL Server 2019以前から存在する9種類の固定ロールは以下のとおりだ。
...
### SQL Server 2022で追加された固定サーバーロール（##MS_系）

SQL Server 2022（16.x）で新たに10種類の固定サーバーロールが追加された。
`##MS_` プレフィックスと `##` サフィックスで通常のロールと区別され、**最小権限の原則（Principle of Least Privilege）に基づいて設計**されている点が特徴だ。

従来の `sysadmin` や `securityadmin` はインスタンス全体への影響が大きすぎるため、これらの新ロールを組み合わせることで必要最小限の権限付与が実現できる。

> **ポイント**: `##MS_`系ロールはサーバーレベルの権限を持ちつつ、DBレベルの権限も継承して伝播する。ただし継承が有効になるのは、ログインがそのDBにユーザーとして登録されているか、`##MS_DatabaseConnector##` のメンバーである場合に限られる。

> **注意**: これらのロールは **SQL Server 2022以降** でのみ利用可能だ。2019以前の環境には存在しない。また、`##MS_PerformanceDefinitionReader##`・`##MS_ServerPerformanceStateReader##`・`##MS_ServerSecurityStateReader##` の3つは Azure SQL Database では利用できない。

---

#### 接続・管理系ロール

DBへの接続やログイン・DB管理を担う3種類のロールは以下のとおりだ。

| ロール名 | サーバーレベル権限 | DB継承権限 | 概要 |
|---|---|---|---|
| `##MS_DatabaseConnector##` | `CONNECT ANY DATABASE` | `CONNECT` | UserMappingなしで全DBに接続可能。特定DBを拒否する場合はそのDB内でユーザーを作成し `DENY CONNECT` を付与する。 |
| `##MS_LoginManager##` | `CREATE LOGIN`, `ALTER ANY LOGIN` | なし | ログインの作成・削除・変更が可能。`securityadmin` と異なり GRANT は不可。権限昇格リスクが低い。 |
| `##MS_DatabaseManager##` | `CREATE ANY DATABASE`, `ALTER ANY DATABASE` | `ALTER` | DBの作成・削除が可能。`dbcreator` の後継として推奨。Azure SQL Databaseと同一スクリプトで管理できる利点がある。 |

---

#### サーバー状態・DMV参照系ロール

DMV（動的管理ビュー）の読み取り権限を細分化した4種類は以下のとおりだ。

| ロール名 | サーバーレベル権限 | DB継承権限 | 概要 |
|---|---|---|---|
| `##MS_ServerStateManager##` | `ALTER SERVER STATE`, `VIEW SERVER STATE`, `VIEW SERVER PERFORMANCE STATE`, `VIEW SERVER SECURITY STATE` | `VIEW DATABASE STATE` 他 | DMV全読み取り＋`DBCC FREEPROCCACHE`・`DBCC SQLPERF()` 等の実行も可能。 |
| `##MS_ServerStateReader##` | `VIEW SERVER STATE`, `VIEW SERVER PERFORMANCE STATE`, `VIEW SERVER SECURITY STATE` | `VIEW DATABASE STATE` 他 | 全DMVの読み取り専用。`ALTER` は不可。 |
| `##MS_ServerPerformanceStateReader##` | `VIEW SERVER PERFORMANCE STATE` | `VIEW DATABASE PERFORMANCE STATE` | パフォーマンス系DMVのみ。`##MS_ServerStateReader##` のサブセット。 |
| `##MS_ServerSecurityStateReader##` | `VIEW SERVER SECURITY STATE` | `VIEW DATABASE SECURITY STATE` | セキュリティ系DMVのみ。`##MS_ServerStateReader##` のサブセット。 |

---

#### 定義参照系ロール

カタログビューの読み取り権限を細分化した3種類は以下のとおりだ。

| ロール名 | サーバーレベル権限 | DB継承権限 | 概要 |
|---|---|---|---|
| `##MS_DefinitionReader##` | `VIEW ANY DATABASE`, `VIEW ANY DEFINITION`, `VIEW ANY PERFORMANCE DEFINITION`, `VIEW ANY SECURITY DEFINITION` | `VIEW DEFINITION` 他 | 全カタログビューの読み取りが可能。 |
| `##MS_PerformanceDefinitionReader##` | `VIEW ANY PERFORMANCE DEFINITION` | `VIEW PERFORMANCE DEFINITION` | パフォーマンス定義のカタログビューのみ。`##MS_DefinitionReader##` のサブセット。 |
| `##MS_SecurityDefinitionReader##` | `VIEW ANY SECURITY DEFINITION` | `VIEW SECURITY DEFINITION` | セキュリティ定義のカタログビューのみ。`##MS_DefinitionReader##` のサブセット。 |
...
```sql
-- ログインを ##MS_DatabaseConnector## に追加（全DBへの接続を許可）
ALTER SERVER ROLE ##MS_DatabaseConnector## ADD MEMBER login1;

-- ログインを ##MS_ServerStateReader## に追加（DMV読み取りを許可）
ALTER SERVER ROLE ##MS_ServerStateReader## ADD MEMBER login1;

-- 特定DBへの接続だけ拒否する（##MS_DatabaseConnector## を付与済みの場合）
USE TargetDB;
...
- **SQL Server 2022以降は `##MS_`系ロールを積極活用する** — `sysadmin` や `securityadmin` の代わりに用途別のロール（`##MS_LoginManager##`・`##MS_DatabaseManager##` 等）を使うことで、最小権限の原則を守りやすくなる。`##MS_DatabaseConnector##` を使えば UserMapping なしで全DBへの接続を一括許可でき、管理コストも下がる
...
- [サーバーレベルのロール — Microsoft Learn](https://learn.microsoft.com/ja-jp/sql/relational-databases/security/authentication-access/server-level-roles)
